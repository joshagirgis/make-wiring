...
void CN0349::calibrateCN0349(char state, float* GF, float* GF_rtd, float* NOS) { //find, and save gain factors, and offsets. See AD5934 and CN-0349 datasheets for process
  ...
  if (state == 'H') { //high measurement rfb = r9
  H1Tmag = calibrate(R3, R3);     //High range 1
  Serial.println("High Magnitude 1 Found");
  Serial.println(H1Tmag, 30);
  H2mag = calibrate(R4, R3);     //High range 2
  Serial.println("High Magnitude 2 Found");
  ...
  *GF_rtd=GF_high;
  *NOS=NOS_high;
  Serial.println(*GF, 30);
  Serial.println(*NOS, 30);
  ...
  *GF_rtd=GF_high;
  ...
  L1mag =  calibrate(R4, R4);    //Low range 1 //was 1000 1000
  Serial.println("Low Magnitude 1 Found");
  Serial.println(L1mag, 30);
  L2mag =  calibrate(R7, R4);   //Low range 2
  Serial.println("Low Magnitude 2 Found");
